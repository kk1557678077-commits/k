...
import type { DisplayHomepageContent, DisplayProduct } from "@/sanity/lib/content";

export function HomeClient({ homepageContent }: { homepageContent: DisplayHomepageContent }) {
  const { lang } = useLanguage();
  const t = ui[lang];
  const home = pages.home[lang];
  const cmsHome = homepageContent[lang];
    ...
    <>
      ...
      <section className="section bg-slate-50">
        <div className="container-page grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
          <div>
            <SectionHeading title={home.inquiryTitle} text={cmsHome.finalCtaSubtitle} />
      ...
      <section className="section">
        <div className="container-page">
          <div className="grid gap-6 rounded-lg bg-navy p-8 text-white shadow-soft md:grid-cols-[1fr_auto] md:items-center">
            <div>
              <h2 className="text-3xl font-bold tracking-tight">{cmsHome.finalCtaTitle}</h2>
              <p className="mt-3 max-w-3xl text-sm leading-6 text-white/75">{cmsHome.finalCtaSubtitle}</p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row md:flex-col">
              <ButtonLink href="/contact" variant="secondary">{cmsHome.secondaryButtonText}</ButtonLink>
              <ButtonLink href="/contact" variant="secondary">{cmsHome.primaryButtonText}</ButtonLink>
